...
def plot_estimator_performance_intermediate(prediction, label_data, label_type,
    ...
    results_file = os.path.join(output_json_dir, 'results.json')
    ...
    # Function to append to specific key in JSON
    def append_to_json_key(filepath, key, new_data):
        with open(filepath, 'r') as fp:
            data = json.load(fp)
        
        # Convert numpy arrays to lists if needed
        if hasattr(new_data, 'tolist'):
    ...
    for i in iterobject:
        ...
        print("PIDs: " + str(test_patient_IDs))
        ...
        if output in ['decision', 'scores'] or crossval_type == 'LOO':
            ...
            # In your loop:
            append_to_json_key(results_file, 'y_scores', y_score)
            append_to_json_key(results_file, 'y_truths', y_truth)
            append_to_json_key(results_file, 'y_predictions', y_prediction)
            append_to_json_key(results_file, 'pids', test_patient_IDs)
...
def main(exp_name, modality, version, use_WIR=False, save_dir='WORC_COM_OS_tmp'):
    ...
    y_truths, y_scores, y_predictions, pids =\
        plot_estimator_performance_intermediate(prediction=prediction,
